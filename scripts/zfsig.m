function [normzf,zf,gci,es,winLength] = zfsig(wav,fs,winLength)
...
function [idx]=xcorrWinLen(wav,fs)
	...
	if(length(wav) < 30*fs/1000)
		...
		[res,lpcc] = LPresidual_v3(wav,frameSize,frameShift,10,0,0);
		hilenv = hilbertenv(res,0);
		wav = hilenv;
