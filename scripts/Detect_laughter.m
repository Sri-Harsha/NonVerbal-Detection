function [] = Detect_laughter(datadir,outdir)

foldet = dir(fullfile(datadir,'*.wav'));
folnam = {foldet.name};
nf = length(folnam);
...
for fileno = 1 : nf
    ...
    filename = sprintf('%s%s',datadir,fname);
    fnam = fname(1:end - 4);
    ...
    [vnv,~,~]=voice_decision(speech1,fs,10);
    vnv = vnv';
    % figure;
    % plot(speech1,'k');
    % hold on
    ...
    [~, ~, ~, gci, st, ~, fs, voiced_regions, ~]=gci_location(speech2,fs1,vnv);		%for demo
