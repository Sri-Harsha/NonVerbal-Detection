...
%	gci=find(diff((zfSig>0))==1); % +ve zero crossings
%	es=abs(zfSig(gci+1)-zfSig(gci-1));

%	figure;
%	ax(1)=subplot(311);plot([1:N]/fs,wav,'k');xlim([0 N]/fs);grid;
%	ax(2)=subplot(312);plot([1:N]/fs,zfSig/max(zfSig),'k');xlim([0 N]/fs);ylim([-1 1]);grid;
%	ax(3)=subplot(313);stem(gci/fs,es,'k');xlim([0 N]/fs);grid;
%	linkaxes(ax,'x');
